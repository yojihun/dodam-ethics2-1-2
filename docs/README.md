...
- 상세 규격 및 예시는 [데이터 스키마 템플릿 가이드(data_schema_template.md)](./data_schema_template.md)를 참고하세요.

### 2단계: TTS 음성 MP3 생성 (`generate_audio.py` 실행)
- 교과서의 모든 설명 텍스트를 기기 구애 없이 맑은 한국어 성우 음성으로 듣기 위해 빌드 타임에 MP3를 자동 추출합니다.
- 상세 실행 방법은 [정적 TTS 오디오 빌드 가이드(audio_generation_utility.md)](./audio_generation_utility.md)를 참고하세요.
...
1. **[데이터 규격 및 교과 데이터 템플릿](./data_schema_template.md)**
   - `chapters.js` 데이터의 JSON 형식 상세 명세 및 수학/영어/과학 과목별 작성 예시 제공.
2. **[정적 TTS MP3 음성 빌드 유틸리티](./audio_generation_utility.md)**
   - `gTTS` 기반의 텍스트 추출 및 MD5 파일명 변환 스크립트 코드와 실행 환경 구축 방법 설명.
3. **[AI 아키텍처 및 핵심 컴포넌트 연동](./component_architecture.md)**
